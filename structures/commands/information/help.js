const { Client, Message, EmbedBuilder, ActionRowBuilder, SelectMenuBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const collector = message.channel.createMessageComponentCollector({ filter, time: 240000 });
        ...
        collector.on('end', collected => {
            if (collected.size === 0) {
                message.editReply({ content: 'Help command timed out.', components: [] });
            }
        });
    },
};
...
function getFiltersEmbed() {
    return new EmbedBuilder()
        ...
        .addFields(
            { name: '/add-filter', value: 'Add a filter' },
            { name: '/remove-filter', value: 'Remove a filter' }
        );
}
